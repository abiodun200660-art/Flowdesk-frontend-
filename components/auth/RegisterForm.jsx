...
export default function RegisterForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    setErrors({})
    try {
      // AuthContext register accepts { name, email, password }
      // Backend: POST /api/auth/register → creates user + default workspace → sets cookie → returns { user }
  ...
  return (
    <div className="animate-fade-in">
      ...
      <p className="text-gray-500 dark:text-gray-400 mb-8">
        Already have one?{' '}
        <Link href="/login" className="text-brand-500 hover:text-brand-600 hover:underline font-semibold transition-colors">
          Sign in
        </Link>
      </p>
      ...
      <form onSubmit={handleSubmit} className="space-y-4" noValidate>
        ...
        <div>
          ...
          <div className="relative">
            ...
            <input
              ...
              className={`input pl-9 ${errors.name ? 'border-red-400 dark:border-red-500 focus:ring-red-500/20 focus:border-red-400' : ''}`}
            />
          </div>
          {errors.name && (
        ...
        <div>
          ...
          <div className="relative">
            ...
            <input
              ...
              className={`input pl-9 ${errors.email ? 'border-red-400 dark:border-red-500 focus:ring-red-500/20 focus:border-red-400' : ''}`}
            />
          </div>
          {errors.email && (
            <p className="flex items-center gap-1.5 mt-1.5 text-xs text-red-500">
        ...
        <div>
          ...
          <div className="relative">
            ...
            <input
              ...
              className={`input pl-9 pr-10 ${errors.password ? 'border-red-400 dark:border-red-500 focus:ring-red-500/20 focus:border-red-400' : ''}`}
            />
            <button
              type="button"
        ...
        <div>
          ...
          <div className="relative">
            ...
            {/* Match checkmark */}
            {form.confirm && form.confirm === form.password && (
              <CheckCircle2 size={15} className="absolute right-9 top-1/2 -translate-y-1/2 text-green-500" />
            )}
          </div>
          {errors.confirm && (
            <p className="flex items-center gap-1.5 mt-1.5 text-xs text-red-500">
